...
if [[ "$VENV_DIR" == "$PRIMARY_VENV_DIR" ]] && [[ -d "$PRIMARY_VENV_DIR" ]] && [[ ! -w "$PRIMARY_VENV_DIR" ]]; then
...
echo "Writing $ENV_FILE (interactive)..."
{
  ...
  echo
  prompt "GITHUB_OWNER" "GitHub owner/org (required)"
...
echo
LOCAL_LLM_MODEL_INSTALL_DEFAULT="auto"
...
run_as_install_user bash "$REPO_ROOT/scripts/setup_local_llm_ollama.sh" "$LOCAL_LLM_MODEL_INSTALL"
